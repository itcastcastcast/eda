...
double meancluster(int K, double bound) {
    ...
    for (auto& entry : blocks) { // 使用非 const 引用以便修改 block
        ...
        for (int i = 0; i < block_k; ++i) {
            ...
            initial_centers.push_back(block_ff[random_index]);
        }
        ...
        while (changed) {
            ...
            for (int i = 0; i < block_k; ++i) {
                if (!block->clusters[i]->members.empty()) {
                    ...
                    if (new_x != initial_centers[i]->x || new_y != initial_centers[i]->y) {
...
void clearall()
{
    ...
    if (best_cluster != nullptr) {
        ...
        {
            cout<<"removed_nodes.size()>=15*exceed"<<endl;
            factor=3;
        ...
        {
            cout<<"removed_nodes.size()>=8*exceed"<<endl;
            factor=2;
            bound=updatebound(bound,factor);
            continue;
        }
        ...
        {
            cout<<"removed_nodes.size()<=4*exceed"<<endl;
            factor=1;
            ...
            if(removed_nodes.size()<exceed)
            {
                ...
                for(auto it = removed_nodes.begin(); it != removed_nodes.end(); ) {
                    ...
                    for(int i = 0; i < var_clusters.size(); ++i) {
                        ...
                        if(distance < min_distance && distance < bound && var_clusters[i]->members.size() < constraint.max_fanout) {
                            min_distance = distance;
                            best_cluster_idx = i;
                        }
                    }
